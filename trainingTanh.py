...
learningParam = 0.1 # ASSIGN THE LEARNING PARAMETER
...
outputNodes = 2 * ((outputNodes - outputMin) / (outputMax - outputMin)) - 1

def tanH(weightedSum):
    # return 1 / (1 + np.exp(-weightedSum)) # Uj = 1 / 1 + e^Sj
    return np.tanh(weightedSum) #BULT IN TAN H FUNCTION

# CREATE THE SIGMOID DERIVATIVE FUNCTION
def tanHDerivative(nodeValue):
    # return nodeValue * (1 - nodeValue) # Uj (1 - Uj)
    return 1 - (tanH(nodeValue) ** 2)
...
for epoch in range(epochs):
    ...
    for i in range(len(inputNodes)):
        ...
        Uj = tanH(weightedSum) # CALCULATE THE SIGMOID FUNCTION USING THE WIEGHTED SUM. THESE ARE THE NEW NODE VALUES FOR THE HIDDEN NODES.
        ...
        predictedOutput = tanH(predictedOutputWeightedSum)
        ...
        outputFirstDerivative = tanHDerivative(predictedOutput) # CALCULATE THE FIRST DERIVATIVE OF THE OUTPUT NODE
        ...
        for j in range(len(Uj)):
            hiddenNodeFirstDerivative = tanHDerivative(Uj[j]) # CALCULATE THE FIRST DERIVATIVE OF THE HIDDEN NODES
...
validationOutputNodes = 2 * ((validationOutputNodes - outputMin) / (outputMax - outputMin)) - 1
totalError = 0
predictedList = []
...
for i in range(len(validationInputNodes)):
    ...
    weightedSum = np.round(weightedSum, 4)
    Uj = tanH(weightedSum) # CALCULATE THE SIGMOID FUNCTION USING THE WEIGHTED SUM. THESE ARE THE NEW NODE VALUES FOR THE HIDDEN NODES.
    ...
    predictedOutput = tanH(predictedOutputWeightesSum)
...
testOutputNodes = 2 * ((testOutputNodes - outputMin) / (outputMax - outputMin)) - 1
